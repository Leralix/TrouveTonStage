...
class WTTJScraper:
    ...
    def launch_scraping(self):
        ...
        try:
            ...
            if self.url_csv != None:
                url_list1 = pd.read_csv(self.url_csv)['url']
                self.scrap(url_list1)
            else:
    ...
    def initialisation_(self,webdriver_path):
    ...
    def offers_links_(self,nb_pages):
        ...
        for i in range(1,nb_pages+1):
            ...
            # Changement de pages
            try:
                changerPageSupp = self.chrome.find_element("xpath","//ul/li[contains(@class,'ais-Pagination-item ais-Pagination-item--nextPage')]")
            except NoSuchElementException:
                break
            df_url.to_csv('temp_wttj_links.csv')
    ...
    def scrap_(self,links):
        ...
        for url in links[:2]:
            ...
            if (self.update_every !=0) and (count % self.update_every==0):
                self.df.to_csv('temp_wttj_offer.csv',index=False)
